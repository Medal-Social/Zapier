...
// For optional fields that were guarded by truthiness before the flag was
// flipped. Unlike `isEmptyValue` this also excludes `false` and `0`, which those
// guards always skipped — scheduling a post on a non-datetime would fire a second
// request that fails only after the post already exists. Payload builders must
// keep using `isEmptyValue` instead: their filter always kept `0`, and a deal
// worth 0 is a real value.
const isProvided = (value) => Boolean(value) && !isEmptyValue(value);
